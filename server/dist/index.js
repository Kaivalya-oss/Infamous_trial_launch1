...
app.post('/api/checkout/process', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        for (const item of items) {
            const variantRes = await client.query('SELECT id, price, stock FROM product_variants WHERE id = $1 FOR UPDATE', [item.variant_id]);
        ...
        const orderRes = await client.query(`INSERT INTO orders (order_number, user_id, total_amount, status, shipping_address) 
       VALUES ($1, $2, $3, $4, $5) RETURNING id`, [orderNumber, userId, totalAmount, 'CONFIRMED', JSON.stringify(address)]);
        const orderId = orderRes.rows[0].id;
        // Insert order items
        for (const item of items) {
            const vRes = await client.query('SELECT sku, price FROM product_variants WHERE id = $1', [item.variant_id]);
            if (vRes.rows.length > 0) {
                await client.query('INSERT INTO order_items (order_id, variant_id, product_name, sku, price, quantity) VALUES ($1, $2, $3, $4, $5, $6)', [orderId, item.variant_id, item.name || 'Product', vRes.rows[0].sku, vRes.rows[0].price, item.quantity]);
            }
        }
        // Payment record insertion
        await client.query('INSERT INTO payments (order_id, user_id, payment_method, amount, status) VALUES ($1, $2, $3, $4, $5)', [orderId, userId, paymentMethod || 'COD', totalAmount, paymentMethod === 'COD' ? 'PENDING' : 'SUCCESS']);
        // Status history
    ...
    try {
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`, [finalName, finalSlug, short_description || '', description || '', categoryIdVal, brand || '', status || 'DRAFT', seo_title || '', seo_description || '']);
        ...
        if (variants && Array.isArray(variants)) {
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [productId, v.sku || null, v.color || '', v.size || '', v.price || 0, v.stock || 0, v.status || 'ACTIVE']);
            }
        }
        if (media && Array.isArray(media)) {
            for (const m of media) {
                await client.query(`INSERT INTO product_images (product_id, cloudinary_url, is_cover)
           VALUES ($1, $2, $3)`, [productId, m.cloudinary_url || '', m.is_cover || false]);
            }
        }
        await client.query('COMMIT');
    ...
    try {
       ...
       WHERE id = $10 RETURNING *`, [finalName, finalSlug, short_description || '', description || '', categoryIdVal, brand || '', status || 'DRAFT', seo_title || '', seo_description || '', id]);
        ...
        if (variants && Array.isArray(variants)) {
            for (const v of variants) {
                await client.query(`INSERT INTO product_variants (product_id, sku, color, size, price, stock, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, v.sku || null, v.color || '', v.size || '', v.price || 0, v.stock || 0, v.status || 'ACTIVE']);
            }
        }
        if (media && Array.isArray(media)) {
            for (const m of media) {
                await client.query(`INSERT INTO product_images (product_id, cloudinary_url, is_cover)
           VALUES ($1, $2, $3)`, [id, m.cloudinary_url || '', m.is_cover || false]);
            }
        }
        await client.query('COMMIT');
